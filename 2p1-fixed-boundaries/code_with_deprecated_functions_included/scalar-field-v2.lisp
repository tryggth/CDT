...
(defun generate-data (infilename mass numthermal numsweeps save-every-n-sweeps)
...
total number of sweeps."
  ...
  (dotimes (ns numthermal)
    (sweep)
    (when (= 0 (mod ns 100))
      (format t "finished thermalizing sweep ~A of ~A for ~A with m = ~A at ~A~%"
	      ns numthermal infilename m (cdt-now-str))))
  ...
  (let* ((dotpos (position #\. infilename :from-end t))
    ...
    (let ((outfilename (concatenate 'string 
				    ...
				    "-saving-every-"
				    (format nil "~A" save-every-n-sweeps)
				    ".sf2p1"))
