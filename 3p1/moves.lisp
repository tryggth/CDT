...
(defun 2->8 (sxid)
  (let ((subcomplex (2->8-subcomplex sxid)))
    (when subcomplex
      (let* ((14id (first (first subcomplex)))
	     (41id (second (first subcomplex)))
	     ...
	     (newids (list (make-4simplex 1 14tmlo 14tmhi lopt newpt 
					  (first tetra) (second tetra) (third tetra))
			   (make-4simplex 1 14tmlo 14tmhi lopt newpt 
					  (second tetra) (third tetra) (fourth tetra))
			   (make-4simplex 1 14tmlo 14tmhi lopt newpt 
					  (third tetra) (fourth tetra) (first tetra))
			   (make-4simplex 1 14tmlo 14tmhi lopt newpt 
					  (fourth tetra) (first tetra) (second tetra))
			   (make-4simplex 4 41tmlo 41tmhi (first tetra) (second tetra) (third tetra)
					  newpt hipt)
			   (make-4simplex 4 41tmlo 41tmhi (second tetra) (third tetra) (fourth tetra)
					  newpt hipt)
			   (make-4simplex 4 41tmlo 41tmhi (third tetra) (fourth tetra) (first tetra)
					  newpt hipt)
			   (make-4simplex 4 41tmlo 41tmhi (fourth tetra) (first tetra) (second tetra)
					  newpt hipt))))
	(connect-4simplices-within-list newids)
	(connect-4simplices-across-lists newids nbors)
	(remove-3simplex oldint3sx)
	(remove-4simplex 14id)
	(remove-4simplex 41id)
	(update-f-vector 28DF)
	(return-from 2->8 T)))))
;;;------------------------------------------------------------------------------------------------------[9]
;;; 8->2
;;;
...
(defun 8->2 (sxid)
  (dolist (subcx (8->2-subcomplex sxid))
	...
	(let ((newids (list (make-4simplex-v2 1 14tmlo 14tmhi (cons lopt (3sx-points newint3sx)))
			    (make-4simplex-v2 4 41tmlo 41tmhi (append (3sx-points newint3sx)(list hipt))))))
	  (connect-4simplices-within-list newids)
	  (connect-4simplices-across-lists newids nbors)
	  (remove-3simplices oldint3sxs)
	  (remove-4simplices subcx)
	  (update-f-vector 82DF)
	  (return-from 8->2 T))))))
;;;------------------------------------------------------------------------------------------------------[1]
;;; 4->6
;;;
...
(defun 4->6 (sxid)
  (dolist (subcx (4->6-subcomplex sxid))
    (let* ((14id1 (first subcx)) (14id2 (second subcx)) (41id2 (third subcx)) (41id1 (fourth subcx))
	   ...
	   (oldint3sxid1 (link-id 14id1 14id2)) (oldint3sxid2 (link-id 41id1 41id2))
	   (oldint3sxid3 (link-id 14id1 41id1)) (oldint3sxid4 (link-id 14id2 41id2))
	   (pts26 (set-exclusive-or (3sx-points (get-3simplex oldint3sxid3)) 
				    (3sx-points (get-3simplex oldint3sxid4))))
	   (pts345 (3sx-hipts (get-3simplex oldint3sxid1)))
	   (pt1 (3sx-lopts (get-3simplex oldint3sxid1)))
	   (pt7 (3sx-hipts (get-3simplex oldint3sxid2)))
	...
	(let ((newids (list (make-4simplex-v2 1 14tmlo 14tmhi (append pt1 (3sx-points newint3sx1)))
			    (make-4simplex-v2 1 14tmlo 14tmhi (append pt1 (3sx-points newint3sx2)))
			    (make-4simplex-v2 1 14tmlo 14tmhi (append pt1 (3sx-points newint3sx3)))
			    (make-4simplex-v2 4 41tmlo 41tmhi (append (3sx-points newint3sx1) pt7))
			    (make-4simplex-v2 4 41tmlo 41tmhi (append (3sx-points newint3sx2) pt7))
			    (make-4simplex-v2 4 41tmlo 41tmhi (append (3sx-points newint3sx3) pt7)))))
	  (connect-4simplices-within-list newids)
	  (connect-4simplices-across-lists newids nbors)
	  (remove-3simplex oldint3sxid1) (remove-3simplex oldint3sxid2)
	  (remove-3simplex oldint3sxid3) (remove-3simplex oldint3sxid4)
	  (remove-4simplices subcx)
	  (update-f-vector 46DF)
	  (return-from 4->6 T))))))
;;;------------------------------------------------------------------------------------------------------[8]
;;; 6->4
;;;
...
(defun 6->4 (sxid)
  (dolist (subcx (6->4-subcomplex sxid))
    (let* ((14id1 (first subcx)) (14id2 (second subcx)) (14id3 (third subcx))
	   ...
	   (oldint3sxid1 (link-id 14id1 41id1)) (oldint3sxid2 (link-id 14id2 41id2))
	   (oldint3sxid3 (link-id 14id3 41id3)) (oldint3sxid4 (link-id 14id1 14id3))
	   (oldint3sxid5 (link-id 14id1 14id2)) (oldint3sxid6 (link-id 14id2 14id3))
	   (oldint3sxid7 (link-id 41id1 41id3)) (oldint3sxid8 (link-id 41id1 41id2))
	   (oldint3sxid9 (link-id 41id2 41id3))
	   (pts26 (intersections (3sx-points (get-3simplex oldint3sxid1))
				 (3sx-points (get-3simplex oldint3sxid2))
				 (3sx-points (get-3simplex oldint3sxid3))))
	   (pts345 (set-difference (unions (3sx-points (get-3simplex oldint3sxid1))
					   (3sx-points (get-3simplex oldint3sxid2))
					   (3sx-points (get-3simplex oldint3sxid3)))
				   pts26))
	...
	(let ((newids (list (make-4simplex-v2 1 14tmlo 14tmhi (append pt1 (3sx-points newint3sx1)))
	  ...
	  (connect-4simplices-within-list newids)
	  (connect-4simplices-across-lists newids nbors)
	  (remove-3simplices (list oldint3sxid1 oldint3sxid2 oldint3sxid3 oldint3sxid4 oldint3sxid5
				   oldint3sxid6 oldint3sxid7 oldint3sxid8 oldint3sxid9))
	  (remove-4simplices subcx)
	  (update-f-vector 64DF)
	  (return-from 6->4 T))))))
;;;------------------------------------------------------------------------------------------------------[2]
;;; 2->4-v1
;;;
...
(defun 2->4-v1-internal-12 (14id 23id)
  "the 2->4-v1 move performed on a (1,4) simplex attached to a (2,3) simplex"
  (let ((14sx nil) (23sx nil))
	...
	(unless (or (gethash newint3sx1 *3SIMPLEX->ID*) (gethash newint3sx2 *3SIMPLEX->ID*)
		    (gethash newint3sx3 *3SIMPLEX->ID*) (gethash newint3sx4 *3SIMPLEX->ID*)
		    (gethash newint3sx5 *3SIMPLEX->ID*) (gethash newint3sx6 *3SIMPLEX->ID*))
	  (let ((newids (list (make-4simplex-v2 1 tmlo tmhi (append pt2 (4sx-hipts 14sx)))
			      (make-4simplex-v2 2 tmlo tmhi (append pts12 (3sx-hipts newint3sx1)))
			      (make-4simplex-v2 2 tmlo tmhi (append pts12 (3sx-hipts newint3sx2)))
			      (make-4simplex-v2 2 tmlo tmhi (append pts12 (3sx-hipts newint3sx3))))))
	    (connect-4simplices-within-list newids)
	    (connect-4simplices-across-lists newids nbors)
	    (remove-3simplex oldint3sxid)
	    (remove-4simplex 14id) (remove-4simplex 23id)
	    (update-f-vector 24DF)
	    (return-from 2->4-v1-internal-12 T)))))))
(defun 2->4-v1-internal-43 (41id 32id)
    "the 2->4-v1 move performed on a (4,1) simplex attached to a (3,2) simplex"
  (let ((41sx nil) (32sx nil))
	...
	(unless (or (gethash newint3sx1 *3SIMPLEX->ID*) (gethash newint3sx2 *3SIMPLEX->ID*)
		    (gethash newint3sx3 *3SIMPLEX->ID*) (gethash newint3sx4 *3SIMPLEX->ID*)
		    (gethash newint3sx5 *3SIMPLEX->ID*) (gethash newint3sx6 *3SIMPLEX->ID*))
	  (let ((newids (list (make-4simplex-v2 4 tmlo tmhi (append (4sx-lopts 41sx) pt2))
			      (make-4simplex-v2 3 tmlo tmhi (append (3sx-lopts newint3sx1) pts12))
			      (make-4simplex-v2 3 tmlo tmhi (append (3sx-lopts newint3sx2) pts12))
			      (make-4simplex-v2 3 tmlo tmhi (append (3sx-lopts newint3sx3) pts12)))))
	    (connect-4simplices-within-list newids)
	    (connect-4simplices-across-lists newids nbors)
	    (remove-3simplex oldint3sxid)
	    (remove-4simplex 41id) (remove-4simplex 32id)
	    (update-f-vector 24DF)
	    (return-from 2->4-v1-internal-43 T)))))))
(defun 2->4-v1 (sxid)
  (let ((subcmplx (2->4-v1-subcomplex sxid)))
    (unless (null subcmplx)
      (dolist (curr subcmplx)
	(cond ((= 12 (first curr))
	       (if (2->4-v1-internal-12 (second curr) (third curr))
		   (return-from 2->4-v1 T)))
	      ((= 43 (first curr))
	       (if (2->4-v1-internal-43 (second curr) (third curr))
		   (return-from 2->4-v1 T))))))))
;;;------------------------------------------------------------------------------------------------------[7]
;;; 4->2-v1
;;;
...
(defun 4->2-v1-internal-1222 (14id 23id1 23id2 23id3)
  (let ((14sx nil) (23sx1 nil) (23sx2 nil) (23sx3 nil))
    (when (and (setf 14sx (get-4simplex 14id)) (setf 23sx1 (get-4simplex 23id1))
      ...
      (let* ((oldint3sxid1 (link-id 14id 23id1))
	     (oldint3sxid2 (link-id 14id 23id2))
	     (oldint3sxid3 (link-id 14id 23id3))
	     (oldint3sxid4 (link-id 23id1 23id2))
	     (oldint3sxid5 (link-id 23id2 23id3))
	     (oldint3sxid6 (link-id 23id3 23id1))
	...
	(unless (gethash newint3sx *3SIMPLEX->ID*)
	  (let ((newids (list (make-4simplex-v2 1 tmlo tmhi (append pt1 (4sx-hipts 14sx)))
			      (make-4simplex-v2 2 tmlo tmhi (append pts12 (3sx-hipts newint3sx))))))
	    (connect-4simplices-within-list newids)
	    (connect-4simplices-across-lists newids nbors)
	    (remove-3simplices (list oldint3sxid1 oldint3sxid2 oldint3sxid3 
				     oldint3sxid4 oldint3sxid5 oldint3sxid6))
	    (remove-4simplices (list 14id 23id1 23id2 23id3))
	    (update-f-vector 42DF)
	    (return-from 4->2-v1-internal-1222 T)))))))
(defun 4->2-v1-internal-4333 (41id 32id1 32id2 32id3)
  (let ((41sx nil) (32sx1 nil) (32sx2 nil) (32sx3 nil))
    (when (and (setf 41sx (get-4simplex 41id)) (setf 32sx1 (get-4simplex 32id1))
      ...
      (let* ((oldint3sxid1 (link-id 41id 32id1))
	     (oldint3sxid2 (link-id 41id 32id2))
	     (oldint3sxid3 (link-id 41id 32id3))
	     (oldint3sxid4 (link-id 32id1 32id2))
	     (oldint3sxid5 (link-id 32id2 32id3))
	     (oldint3sxid6 (link-id 32id3 32id1))
	...
	(unless (gethash newint3sx *3SIMPLEX->ID*)
	  (let ((newids (list (make-4simplex-v2 4 tmlo tmhi (append (4sx-lopts 41sx) pt1))
			      (make-4simplex-v2 3 tmlo tmhi (append (3sx-lopts newint3sx) pts12)))))
	    (connect-4simplices-within-list newids)
	    (connect-4simplices-across-lists newids nbors)
	    (remove-3simplices (list oldint3sxid1 oldint3sxid2 oldint3sxid3 
				     oldint3sxid4 oldint3sxid5 oldint3sxid6))
	    (remove-4simplices (list 41id 32id1 32id2 32id3))
	    (update-f-vector 42DF)
	    (return-from 4->2-v1-internal-4333 T)))))))
(defun 4->2-v1 (sxid)
  (let ((subcmplx (4->2-v1-subcomplex sxid)))
    (unless (null subcmplx)
      (dolist (curr subcmplx)
	(cond ((= 1222 (first curr))
	       (if (4->2-v1-internal-1222 (second curr) (third curr) (fourth curr) (fifth curr))
		   (return-from 4->2-v1 T)))
	      ((= 4333 (first curr))
	       (if (4->2-v1-internal-4333 (second curr) (third curr) (fourth curr) (fifth curr))
		   (return-from 4->2-v1 T))))))))
;;;------------------------------------------------------------------------------------------------------[3]
;;; 2->4-v2
;;;
  ...
  (dolist (subcx (2->4-v2-subcomplex sxid))
    (let* ((23id (first subcx)) (32id (second subcx))
      ...
      (unless (or (gethash newint3sx1 *3SIMPLEX->ID*) (gethash newint3sx2 *3SIMPLEX->ID*)
		  (gethash newint3sx3 *3SIMPLEX->ID*) (gethash newint3sx4 *3SIMPLEX->ID*)
		  (gethash newint3sx5 *3SIMPLEX->ID*) (gethash newint3sx6 *3SIMPLEX->ID*))
	(let ((newids (list (make-4simplex-v2 2 tmlo tmhi (append (3sx-lopts newint3sx3) pts456))
			    (make-4simplex-v2 2 tmlo tmhi (append (3sx-lopts newint3sx5) pts456))
			    (make-4simplex-v2 3 tmlo tmhi (append pts123 (3sx-hipts newint3sx3)))
			    (make-4simplex-v2 3 tmlo tmhi (append pts123 (3sx-hipts newint3sx4))))))
	  (connect-4simplices-within-list newids)
	  (connect-4simplices-across-lists newids nbors)
	  (remove-3simplex oldint3sxid)
	  (remove-4simplex 32id) (remove-4simplex 23id)
	  (update-f-vector 24DF)
	  (return-from 2->4-v2 T))))))
;;;------------------------------------------------------------------------------------------------------[6]
;;; 4->2-v2
;;;
;;;     4 5 6
...
(defun 4->2-v2 (sxid)
  (dolist (subcx (4->2-v2-subcomplex sxid))
    (let* ((23id1 (first subcx)) (23id2 (second subcx))
	   ...
	   (oldint3sxid1 (link-id 23id1 23id2)) (oldint3sxid2 (link-id 32id1 32id2))
	   (oldint3sxid3 (link-id 23id1 32id1)) (oldint3sxid4 (link-id 23id1 32id2))
	   (oldint3sxid5 (link-id 23id2 32id1)) (oldint3sxid6 (link-id 23id2 32id2))
      ...
      (unless (gethash newint3sx *3SIMPLEX->ID*)
	(let ((newids (list (make-4simplex-v2 2 tmlo tmhi (append (3sx-lopts newint3sx) pts456))
			    (make-4simplex-v2 3 tmlo tmhi (append pts123 (3sx-hipts newint3sx))))))
	  (connect-4simplices-within-list newids)
	  (connect-4simplices-across-lists newids nbors)
	  (remove-3simplices (list oldint3sxid1 oldint3sxid2 oldint3sxid3 
				   oldint3sxid4 oldint3sxid5 oldint3sxid6))
...
(defun 3->3-v1-internal-122 (14id 23id1 23id2)
  (let ((14sx nil) (23sx1 nil) (23sx2 nil))
    (when (and (setf 14sx (get-4simplex 14id)) (setf 23sx1 (get-4simplex 23id1))
      ...
      (let* ((oldint3sxid1 (link-id 14id 23id1))
	     (oldint3sxid2 (link-id 14id 23id2))
	     (oldint3sxid3 (link-id 23id1 23id2))
	     (pts12 (4sx-lopts 23sx1))
	     (pt2 (set-difference pts12 (4sx-lopts 14sx)))
	...
	(unless (or (gethash newint3sx1 *3SIMPLEX->ID*)
	  ...
	  (let ((newids (list (make-4simplex-v2 1 tmlo tmhi (append pt2 (4sx-hipts 14sx)))
	    ...
	    (connect-4simplices-across-lists newids nbors)
	    (remove-3simplices (list oldint3sxid1 oldint3sxid2 oldint3sxid3)) 
	    (remove-4simplices (list 14id 23id1 23id2))
	    (update-f-vector 33DF)
	    (return-from 3->3-v1-internal-122 T)))))))
(defun 3->3-v1-internal-433 (41id 32id1 32id2)
  (let ((41sx nil) (32sx1 nil) (32sx2 nil))
    (when (and (setf 41sx (get-4simplex 41id)) (setf 32sx1 (get-4simplex 32id1))
      ...
      (let* ((oldint3sxid1 (link-id 41id 32id1))
	     (oldint3sxid2 (link-id 41id 32id2))
	     (oldint3sxid3 (link-id 32id1 32id2))
	     (pts12 (4sx-hipts 32sx1))
	     (pt2 (set-difference pts12 (4sx-hipts 41sx)))
	...
	(unless (or (gethash newint3sx1 *3SIMPLEX->ID*)
	  ...
	  (let ((newids (list (make-4simplex-v2 4 tmlo tmhi (append (4sx-lopts 41sx) pt2))
	    ...
	    (connect-4simplices-across-lists newids nbors)
	    (remove-3simplices (list oldint3sxid1 oldint3sxid2 oldint3sxid3)) 
	    (remove-4simplices (list 41id 32id1 32id2))
	    (update-f-vector 33DF)
	    (return-from 3->3-v1-internal-433 T)))))))
(defun 3->3-v1 (sxid)
  (let ((subcmplx (3->3-v1-subcomplex sxid)))
    (unless (null subcmplx)
      (dolist (curr subcmplx)
	(cond ((= 122 (first curr))
	       (if (3->3-v1-internal-122 (second curr) (third curr) (fourth curr))
		   (return-from 3->3-v1 T)))
	      ((= 433 (first curr))
	       (if (3->3-v1-internal-433 (second curr) (third curr) (fourth curr))
		   (return-from 3->3-v1 T))))))))
;;;------------------------------------------------------------------------------------------------------[5]
;;; 3->3-v2
;;;
...
(defun 3->3-v2-internal-223 (23id1 23id2 32id)
  (let ((23sx1 nil) (23sx2 nil) (32sx nil))
    (when (and (setf 23sx1 (get-4simplex 23id1)) (setf 23sx2 (get-4simplex 23id2))
      ...
      (let* ((oldint3sxid1 (link-id 23id1 23id2))
	     (oldint3sxid2 (link-id 23id1 32id))
	     (oldint3sxid3 (link-id 23id2 32id))
	...
	(unless (or (gethash newint3sx1 *3SIMPLEX->ID*)
		    (gethash newint3sx2 *3SIMPLEX->ID*)
		    (gethash newint3sx3 *3SIMPLEX->ID*))
	  (let ((newids (list (make-4simplex-v2 2 tmlo tmhi (append pts23 pts456))
			      (make-4simplex-v2 3 tmlo tmhi (append pts123 (3sx-hipts newint3sx1)))
			      (make-4simplex-v2 3 tmlo tmhi (append pts123 (3sx-hipts newint3sx2))))))
	    (connect-4simplices-within-list newids)
	    (connect-4simplices-across-lists newids nbors)
	    (remove-3simplices (list oldint3sxid1 oldint3sxid2 oldint3sxid3)) 
	    (remove-4simplices (list 23id1 23id2 32id))
	    (update-f-vector 33DF)
	    (return-from 3->3-v2-internal-223 T)))))))
(defun 3->3-v2-internal-332 (32id1 32id2 23id)
  (let ((32sx1 nil) (32sx2 nil) (23sx nil))
    (when (and (setf 32sx1 (get-4simplex 32id1)) (setf 32sx2 (get-4simplex 32id2))
      ...
      (let* ((oldint3sxid1 (link-id 32id1 32id2))
	     (oldint3sxid2 (link-id 32id1 23id))
	     (oldint3sxid3 (link-id 32id2 23id))
	...
	(unless (or (gethash newint3sx1 *3SIMPLEX->ID*)
		    (gethash newint3sx2 *3SIMPLEX->ID*)
		    (gethash newint3sx3 *3SIMPLEX->ID*))
	  (let ((newids (list (make-4simplex-v2 3 tmlo tmhi (append pts456 pts23))
			      (make-4simplex-v2 2 tmlo tmhi (append (3sx-lopts newint3sx1) pts123))
			      (make-4simplex-v2 2 tmlo tmhi (append (3sx-lopts newint3sx2) pts123)))))
	    (connect-4simplices-within-list newids)
	    (connect-4simplices-across-lists newids nbors)
	    (remove-3simplices (list oldint3sxid1 oldint3sxid2 oldint3sxid3))
	    (remove-4simplices (list 32id1 32id2 23id))
	    (update-f-vector 33DF)
	    (return-from 3->3-v2-internal-332 T)))))))
(defun 3->3-v2 (sxid)
  (let ((subcmplx (3->3-v2-subcomplex sxid)))
    (unless (null subcmplx)
      (dolist (curr subcmplx)
	(cond ((= 223 (first curr))
	       (if (3->3-v2-internal-223 (second curr) (third curr) (fourth curr))
		   (return-from 3->3-v2 T)))
	      ((= 332 (first curr))
	       (if (3->3-v2-internal-332 (second curr) (third curr) (fourth curr))
		   (return-from 3->3-v2 T))))))))
