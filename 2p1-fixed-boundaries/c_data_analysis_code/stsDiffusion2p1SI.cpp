...
stsDiffusion2p1::stsDiffusion2p1(  const char* infilename, 
...
stsDiffusion2p1::~stsDiffusion2p1( void )
{
}

void stsDiffusion2p1::_loadSpacetimeDataFromFile( void )
{
...
void stsDiffusion2p1::_parseParameterLine( std::string& line )
{
...
void stsDiffusion2p1::_parseDataLine( std::string& line, bignum_t& id, simplex3& sx )
{
  std::string::size_type begIdx = 0, endIdx = line.find_first_of(" ",begIdx);
...
bignum_t stsDiffusion2p1::_countSimplicesInSandwich( fixnum_t tmlo, fixnum_t tmhi )
{
  bignum_t count = 0;
...
void stsDiffusion2p1::_computeMaxVolSandwich( void )
{
...
std::vector<bignum_t> 
stsDiffusion2p1::_getSimplicesInSandwich
...
real_t stsDiffusion2p1::KT( bignum_t j, bignum_t i0, fixnum_t sig )
{
  if ( sig == 0 )
    {
      if( i0 == j )
