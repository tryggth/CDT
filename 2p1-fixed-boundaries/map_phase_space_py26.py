...
(initialize-t-slices-with-v-volume :num-time-slices 64
				   ...
				   :boundary-conditions "OPEN"
                                   :initial-spatial-geometry %s
                                   :final-spatial-geometry %s)

(set-k0-k3-alpha %f %f -1)
...
def tune_k3 (k0,initial_geometry='"tetra.txt"',final_geometry='"tetra.txt"'):
    ...
    for k3 in k3range:
        ...
        with open('tuning.script','w') as f:
            f.write(script % (target_volume,initial_geometry,final_geometry,k0,k3))
        print "k0 = %f. k3 = %f." % (k0,k3)
        # Run the script
...
def vary_k0(initial_geometry='"tetra.txt"',final_geometry='"tetra.txt"'):
    ...
    with open(outfile,'w') as f:
        f.write("Data from tuning: %f <= k0 <= %f\n" % (k0min,k0max))
