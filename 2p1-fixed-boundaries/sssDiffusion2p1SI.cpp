...
sssDiffusion2p1::sssDiffusion2p1(const char* infilename, 
...
sssDiffusion2p1::~sssDiffusion2p1( void )
{
}

void sssDiffusion2p1::_loadDataFromFile( void )
{
...
void sssDiffusion2p1::_parseParameterLine( std::string& line )
{
...
void sssDiffusion2p1::_parseDataLine( std::string& line, bignum_t& id, s2simplex& sx )
{
  std::string::size_type begIdx = 0, endIdx = line.find_first_of(" ",begIdx);
...
void sssDiffusion2p1::_getS2SimplicesInSlice( fixnum_t tslice, 
...
void sssDiffusion2p1::_computeMaxVolSlice( void )
{
...
void sssDiffusion2p1::diffusion(void)
{
  _computeMaxVolSlice();
  ...
  for( size_t ndx = 0; ndx < _numtrials; ++ndx )
    {
      ...
      for(size_t ndx = 0; ndx < 3; ++ndx )
	_KTstore[kt_key_t(sx._nbrids[ndx], i0, 1)] = 1.0/3.0;
...
real_t sssDiffusion2p1::KT( bignum_t j, bignum_t i0, fixnum_t sig )
{
  if ( sig == 0 )
    {
      if( i0 == j )
  ...
  if( _KTstore.find( k ) == _KTstore.end() )
    {
      real_t kt_tot = 0.0;
      s2simplex& sx = _s2sxstore[j];
      for( size_t ndx = 0; ndx < 3; ++ndx )
	{
	  kt_tot += KT( sx._nbrids[ndx], i0, sig-1 );
	}
      _KTstore[k] = kt_tot/3.0;
    }
  return _KTstore[k];
}
