...
(defvar *TIME->NS2SX* (make-hash-table))
(defvar *TIME->N3SX22U* (make-hash-table))
(defvar *TIME->N3SX22D* (make-hash-table))

(defun generate-and-save-HL-spatial-slice (outfile)
  "generates and saves spatial slice information to outfile"

  ;; clear the bookkeeping tables
  (clrhash *TIME->NS2SX*)
  (clrhash *TIME->N3SX22U*)
  (clrhash *TIME->N3SX22D*)

  (maphash #'(lambda (k v)
               (push (nth 0 (gethash k *0SIMPLEX->NS2SX*))
                     (gethash v *TIME->NS2SX*)))
           *0SIMPLEX->TIME*)

  (maphash #'(lambda (k v)
               (when (= 0 (2sx-type k))
                 (let* ((p0 (first (2sx-points k)))
                        (p1 (second (2sx-points k)))
                        (p2 (third (2sx-points k)))
                        (l0 (list p0 p1))
                        (l1 (list p1 p2))
                        (l2 (list p2 p0))
                        (n0 (gethash l0 *1SIMPLEX->N3SX22UD* '(0 0)))
                        (n1 (gethash l1 *1SIMPLEX->N3SX22UD* '(0 0)))
                        (n2 (gethash l2 *1SIMPLEX->N3SX22UD* '(0 0)))
                        (numD (+ (first n0) (first n1) (first n2)))
                        (numU (+ (second n0) (second n1) (second n2))))
                   (push numU (gethash (2sx-tmlo k) *TIME->N3SX22U*)) 
                   (push numD (gethash (2sx-tmlo k) *TIME->N3SX22D*)))))
           *2SIMPLEX->ID*)

  (for (ts 0 (1- NUM-T))
       (format outfile "~A " ts)
       (dolist (num (gethash ts *TIME->NS2SX*))
         (format outfile "~A " num))
       (format outfile "~%")
       (format outfile "~A " ts)
       (dolist (num (gethash ts *TIME->N3SX22U*))
         (format outfile "~A " num))
       (format outfile "~%")
       (format outfile "~A " ts)
       (dolist (num (gethash ts *TIME->N3SX22D*))
         (format outfile "~A " num))
       (format outfile "~%")))