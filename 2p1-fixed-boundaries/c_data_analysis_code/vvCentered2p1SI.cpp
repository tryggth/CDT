...
vvCentered2p1::vvCentered2p1(  const char* infilename, 
			       const char* outfilename )
  : _ifilename(infilename), _ofilename(outfilename)
{
  _loadSpatialSliceDataFromFile();
  _computeMaxVolSlice();
  _vvCorrelations.reserve( 2*_numT + 1 );
}

vvCentered2p1::~vvCentered2p1( void )
{
}

void vvCentered2p1::_loadSpatialSliceDataFromFile( void )
{
...
void vvCentered2p1::_parseParameterLine( std::string& line )
{
...
bignum_t 
vvCentered2p1::_countS2SimplicesInSlice
( fixnum_t tslice ) 
{
  bignum_t count = 0;
...
void 
vvCentered2p1::_computeMaxVolSlice
( void )
{
...
void vvCentered2p1::center( void )
{
  // compute the correlations
  for( fixnum_t i = -_numT/2; i <= _numT/2; ++i )
    {
      _vvCorrelations[i + _numT] = 
	_countS2SimplicesInSlice(1+(i + _maxVolSlice - 1)%_numT);
    }
  ...
  if(fout.is_open())
    {
      for(fixnum_t delta = -_numT/2; delta <= _numT/2; ++delta)
	fout << delta << "\t" << 
	  _vvCorrelations[delta + _numT] << std::endl;
    }
  fout.close();
}
