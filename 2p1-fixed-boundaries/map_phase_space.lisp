...
(defun format-hour-day-month-year ()
  ...
  (let* ((datelist (multiple-value-list (get-decoded-time)))
	 ...
	 (year  (sixth  datelist)))
    (format nil "~ah-~a-~a-~a" hours days month year)))
;;;--------------------------------------------------------------------------
...
;; Various
(defvar *outfile* (format nil "~a.phasemap" (format-hour-day-month-year))
...
puts in the proper information."
  (with-open-file (f outfile 
    ...
    (format f "# k0~Tk3~TN3-mean~Tsigma~%")))

(defun add-data-to-file (outfile k0 k3 n3-mean n3-sigma)
  "Adds the requisite information to the output file."
  (with-open-file (f outfile
		     :direction :output
		     :if-exists :append
		     :if-does-not-exist :create)
    (format f "~3D ~3D ~3D ~3D~%" k0 k3 n3-mean n3-sigma)))
     ...
     (loop for k0 from k0-min to k0-max by k0-increment do
	  (loop for k3 from k3-min to k3-max by k3-increment do
		  ...
		  (setf data (collect-volume-data thermalization-sweeps
						   data-sweeps))
		  (add-data-to-file outfile k0 k3 (float (mean data))
